

IXAudio2* CAudioClip::m_Xaudio;
IXAudio2MasteringVoice*	CAudioClip::m_MasteringVoice;

IXAudio2SourceVoice* CAudioClip::m_SourceVoice[SOUND_SOURCE_MAX];
BYTE* CAudioClip::m_SoundData[SOUND_SOURCE_MAX];

int CAudioClip::m_Length;
int	CAudioClip::m_PlayLength;

int CAudioClip::mNumBGM;

void CAudioClip::Init(void)
{
	// COM初期化
	HRESULT com = CoInitializeEx( NULL, COINIT_MULTITHREADED );

	// XAudio生成
	XAudio2Create( &m_Xaudio, 0 );

	// マスタリングボイス生成
	m_Xaudio->CreateMasteringVoice( &m_MasteringVoice );

	mNumBGM = 0;
}


void CAudioClip::Uninit(void)
{

	m_MasteringVoice->DestroyVoice();

	m_Xaudio->Release();

	CoUninitialize();

}








void CAudioClip::Load(const char *FileName)
{

	// サウンドデータ読込
	WAVEFORMATEX wfx = { 0 };

	HMMIO hmmio = NULL;
	MMIOINFO mmioinfo = { 0 };
	MMCKINFO riffchunkinfo = { 0 };
	MMCKINFO datachunkinfo = { 0 };
	MMCKINFO mmckinfo = { 0 };
	UINT32 buflen;
	LONG readlen;


	hmmio = mmioOpen((LPSTR)FileName, &mmioinfo, MMIO_READ);
	assert(hmmio);

	riffchunkinfo.fccType = mmioFOURCC('W', 'A', 'V', 'E');
	mmioDescend(hmmio, &riffchunkinfo, NULL, MMIO_FINDRIFF);

	mmckinfo.ckid = mmioFOURCC('f', 'm', 't', ' ');
	mmioDescend(hmmio, &mmckinfo, &riffchunkinfo, MMIO_FINDCHUNK);

	if (mmckinfo.cksize >= sizeof(WAVEFORMATEX))
	{
		mmioRead(hmmio, (HPSTR)&wfx, sizeof(wfx));
	}
	else
	{
		PCMWAVEFORMAT pcmwf = { 0 };
		mmioRead(hmmio, (HPSTR)&pcmwf, sizeof(pcmwf));
		memset(&wfx, 0x00, sizeof(wfx));
		memcpy(&wfx, &pcmwf, sizeof(pcmwf));
		wfx.cbSize = 0;
	}
	mmioAscend(hmmio, &mmckinfo, 0);

	datachunkinfo.ckid = mmioFOURCC('d', 'a', 't', 'a');
	mmioDescend(hmmio, &datachunkinfo, &riffchunkinfo, MMIO_FINDCHUNK);



	buflen = datachunkinfo.cksize;
	m_SoundData[mNumBGM] = new unsigned char[buflen];
	readlen = mmioRead(hmmio, (HPSTR)m_SoundData[mNumBGM], buflen);


	m_Length = readlen;
	m_PlayLength = readlen / wfx.nBlockAlign;


	mmioClose(hmmio, 0);

	m_Xaudio->CreateSourceVoice(&m_SourceVoice[mNumBGM], &wfx);
	mNumBGM++;

	// サウンドソース生成
	//for (int j = 0; j < SOUND_SOURCE_MAX; j++)
	//{
	//	m_Xaudio->CreateSourceVoice(&m_SourceVoice[j], &wfx);
	//}
}


void CAudioClip::Unload()
{

	for (int j = 0; j < SOUND_SOURCE_MAX; j++)
	{
		if (m_SourceVoice[j])
		{
			m_SourceVoice[j]->Stop();
			m_SourceVoice[j]->DestroyVoice();
			m_SourceVoice[j] = NULL;
		}

		if (m_SoundData[j])
		{
			delete[] m_SoundData[j];
			m_SoundData[j] = nullptr;
		}

	}
}


void CAudioClip::Play(bool Loop, CBGM bgm)
{
	XAUDIO2_VOICE_STATE state;

	m_SourceVoice[(int)bgm]->GetState(&state);

	state.BuffersQueued = 0;

	if (state.BuffersQueued == 0)
	{
		//Sound->SourceVoice->Stop();
		//Sound->SourceVoice->FlushSourceBuffers();

		// バッファ設定
		XAUDIO2_BUFFER bufinfo;

		memset(&bufinfo, 0x00, sizeof(bufinfo));
		bufinfo.AudioBytes = m_Length;
		bufinfo.pAudioData = m_SoundData[(int)bgm];
		bufinfo.PlayBegin = 0;
		bufinfo.PlayLength = m_PlayLength;

		// ループ設定
		if (Loop)
		{
			bufinfo.LoopBegin = 0;
			bufinfo.LoopLength = m_PlayLength;
			bufinfo.LoopCount = XAUDIO2_LOOP_INFINITE;
		}

		m_SourceVoice[(int)bgm]->SubmitSourceBuffer(&bufinfo, NULL);


		// 再生
		m_SourceVoice[(int)bgm]->Start();
	}
}


void CAudioClip::Play( bool Loop )
{

	for( int j = 0; j < SOUND_SOURCE_MAX; j++ )
	{
		XAUDIO2_VOICE_STATE state;

		m_SourceVoice[j]->GetState( &state );

		if( state.BuffersQueued == 0 )
		{
			//Sound->SourceVoice->Stop();
			//Sound->SourceVoice->FlushSourceBuffers();


			// バッファ設定
			XAUDIO2_BUFFER bufinfo;

			memset(&bufinfo,0x00,sizeof(bufinfo));
			bufinfo.AudioBytes = m_Length;
			bufinfo.pAudioData = m_SoundData[j];
			bufinfo.PlayBegin = 0;
			bufinfo.PlayLength = m_PlayLength;

			// ループ設定
			if( Loop )
			{
				bufinfo.LoopBegin = 0;
				bufinfo.LoopLength = m_PlayLength;
				bufinfo.LoopCount = XAUDIO2_LOOP_INFINITE;
			}

			m_SourceVoice[j]->SubmitSourceBuffer( &bufinfo, NULL );


			// 再生
			m_SourceVoice[j]->Start();

			break;
		}
	}

}

void CAudioClip::Stop(CBGM bgm)
{
	m_SourceVoice[(int)bgm]->Stop();
}

void CAudioClip::StopAll()
{
	for (int j = 0; j < SOUND_SOURCE_MAX; j++)
	{
		m_SourceVoice[j]->Stop();
	}
}


